...
do
	...
	do
		...
		sleep 20
		sudo python ${ebpf_file} ${dist_dir}/${fs}/ebpf/ebpf_${fs}_g${gc_idle}_t${type}_fileserver_${count}.txt 2>/dev/null &
		sleep 10
		sudo killall python
		wait
		...
		sleep 20
		sudo python ${ebpf_file} ${dist_dir}/${fs}/ebpf/ebpf_${fs}_g${gc_idle}_t${type}_webserver_${count}.txt 2>/dev/null &
		sleep 10
		sudo killall python
		wait
		...
		sleep 20
		sudo python ${ebpf_file} ${dist_dir}/${fs}/ebpf/ebpf_${fs}_g${gc_idle}_t${type}_varmail_${count}.txt 2>/dev/null &
		sleep 10
		sudo killall python
		wait
