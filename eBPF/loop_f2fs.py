...
void trace_f2fs_file_write_iter_start(struct pt_regs *ctx) {
	struct blxdata_t blxdata = {};
	blxdata.Start = 'S';
    	blxdata.pid = bpf_get_current_pid_tgid();
    	blxdata.ts = bpf_ktime_get_ns();
    	strcpy(blxdata.fucname, "f2fs_file_write_iter");
    	bpf_get_current_comm(&blxdata.comm, sizeof(blxdata.comm));
    	result.perf_submit(ctx, &blxdata, sizeof(blxdata));
}

void trace_f2fs_file_write_iter_completion(struct pt_regs *ctx) {
...
if __name__ == '__main__':
	...
	b.attach_kprobe(event="f2fs_file_write_iter", fn_name="trace_f2fs_file_write_iter_start")
	b.attach_kretprobe(event="f2fs_file_write_iter", fn_name="trace_f2fs_file_write_iter_completion")
